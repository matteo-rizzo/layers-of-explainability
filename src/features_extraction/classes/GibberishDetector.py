...
class GibberishDetector(Feature):
    ...
    def extract(self, texts: list[str]) -> dict[str, list[float]]:
        ...
        [feature_df[f'{self.__class__.__name__}_{emotion["label"].replace(" ", "_")}'].append(emotion["score"]) for labels_list_dict in
    ...
    def label_description(cls) -> dict[str, str]:
        return {
            ...
            f"{cls.__name__}_mild_gibberish": "text has grammatical errors, word sense errors, or syntactical abnormalities that reduce the overall coherence",
            f"{cls.__name__}_word_salad": "words make sense independently, but do not produce a coherent meaning",
