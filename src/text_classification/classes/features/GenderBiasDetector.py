...
class GenderBiasDetector(Feature):
    def __init__(self, use_gpu: bool = True, batch_size: int = 256, *args, **kwargs):
        self.pipe = pipeline("text-classification", model="thaile/roberta-base-md_gender_bias-saved", device="cuda" if use_gpu else "cpu",
    ...
    def label_description(cls) -> dict[str, str]:
        return {
            f"{cls.__name__}_LABEL_0": "text is gender neutral",
            f"{cls.__name__}_LABEL_1": "text contains gender bias"
        }
