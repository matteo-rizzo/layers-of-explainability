...
def plot_permutation_importance(clf, X, y, ax, scorer: Callable, metric_name: str, rskf: RepeatedStratifiedKFold | None = None):
    def process_fold(train_index, test_index) -> dict[str, float]:
        ...
        return permutation_importance(clf, x_test_data, y_test_data, n_repeats=5, random_state=41, n_jobs=4, scoring=scorer)[metric_name]

    if rskf is not None:
        # Use the validation set and make importance on k-fold validation
        results = Parallel(n_jobs=4)(delayed(process_fold)(train_index, test_index) for train_index, test_index in rskf.split(X, y))
