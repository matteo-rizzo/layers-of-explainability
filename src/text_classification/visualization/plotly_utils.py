...
def visualize_topics_over_time_ext(topic_model,
    ...
    for index, topic in enumerate(data.Topic.unique()):
        ...
        if relative_frequency:
            tot_freq = topics_over_time.groupby("Timestamp")["Frequency"].sum()
            tot_freq.name = "Tot"
            trace_data = pd.merge(trace_data, tot_freq, left_on="Timestamp", right_index=True)
            y = trace_data["Frequency"] / trace_data["Tot"]
        ...
        fig.add_trace(go.Scatter(x=trace_data.Timestamp, y=y,
                                 ...
                                 hoverinfo="text",
                                 name=topic_name,
                                 hovertext=[f'<b>Topic {topic}</b><br>Words: {word}<br>Frequency: {f}' for word, f in zip(words, trace_data.Frequency)]))
