...
class SHAPexplainer:
    ...
    def __init__(self, model, tokenizer, words_dict, words_dict_reverse, device="cuda", use_logits: bool = True, max_tokens: int = 512):
    ...
    def tknz_to_idx(self, train_data, MAX_SEQ_LEN=None):
        tokenized_nopad = [self.tokenizer.tokenize(" ".join(text), max_length=self.model_max_tokens, truncation=True) for text in train_data]
        if not MAX_SEQ_LEN:
