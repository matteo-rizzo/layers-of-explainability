...
from src.datasets.classes.BiasDataset import BiasDataset


class HuggingFaceBiasDataset(BiasDataset):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.hf_data = {k: {"text": v["x"], "label": v["y"]} for k, v in self._split_data.items() if
                        isinstance(v, dict)}
