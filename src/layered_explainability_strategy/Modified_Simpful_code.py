...
def create_system():
    ...
    RULE2 = "IF (offensive IS cluster2) AND (female IS cluster2) AND (TextEmotion_anger IS cluster2) AND (subjectivity IS cluster2) AND (GibberishDetector_clean IS cluster2) AND (sarcasm IS cluster2) AND (word_count IS cluster2) AND (irony IS cluster2) THEN (OUTPUT IS fun2)"
    ...
    if plot:
        MF_TextEmotion_anger.plot(xscale='log')
    ...
    if plot:
        MF_subjectivity.plot(xscale='log')
    ...
    if plot:
        MF_GibberishDetector_clean.plot(xscale='log')
    ...
    MF_word_count = LinguisticVariable([FS_13, FS_14], concept='word_count', universe_of_discourse=u_o_d)
    if plot:
        MF_word_count.plot(xscale='log')
...
if __name__ == "__main__":
    ...
    test = SugenoFISTester(model=fis, test_data=test_dl.dataX, variable_names=test_dl.variable_names, golden_standard=test_dl.dataY)
